...
public class TravelPlanningServiceImpl implements ITravelPlanningService{
	...
	@Override
	public TravelPlanning retrieveTravelPlanningById(Long idPlanning) {
		return planningRepo.findById(idPlanning).orElse(null);
		
	}
	@Override
	public TravelPlanning updateTravelPlanningById(TravelPlanning t, Long idPlanning) {
		TravelPlanning found= planningRepo.findById(idPlanning).orElse(t);
		found.setDay(t.getDay());
		found.setDescription(t.getDescription());
		found.setSchedule(t.getSchedule());
		planningRepo.saveAndFlush(found);
		return found;
